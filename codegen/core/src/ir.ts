...
/**
 * Every SQLite keyword (https://www.sqlite.org/lang_keywords.html, 147
 * entries as of 2026-09-07), lowercased.
 *
 * Used for ONE rule: an inline function may not be NAMED a keyword. A
 * function name reaches a script in call position, and the parser sees
 * the keyword before it ever looks for a function — `SELECT select(1)`
 * is a syntax error, not a missing-function error. The registration
 ...
 * constants, because no runtime or validator needs it.
 */
