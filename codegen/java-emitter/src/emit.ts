...
/** Java package of the host-independent protocol constants (fixed per protocol). */
export const PROTOCOL_PACKAGE = "io.sqlitehost.model";
export const PROTOCOL_FILE = "io/sqlitehost/model/Protocol.java";
...
/**
 * Host-independent protocol constants the Java validator consumes,
 * projected from the single source in codegen/core/src/ir.ts
 * (docs/proposals/rule-parameters-as-data.md). Zero-arg — the values are
 * protocol constants, not per-host.
 */
export function emitJavaProtocolConstants(): EmittedFile {
  const scalarOrder: ScalarTypeIr[] = [
    "int32",
    "int64",
    "boolean",
    "string",
  ...
  const compatEntries = scalarOrder
    .map((scalar) => {
      const accepted = BINDING_TYPE_COMPAT[scalar].map((t) => javaString(t)).join(", ");
      return `            ${javaString(scalar)}, Set.of(${accepted})`;
    })
    .join(",\n");
  ...
  const forbiddenKeywords = wrapJavaArgs(
    FORBIDDEN_LEADING_KEYWORDS.map(javaString),
    "            ",
  );
  const forbiddenFunctions = wrapJavaArgs(
    FORBIDDEN_FUNCTIONS.map(javaString),
    "            ",
  );
  const systemTables = wrapJavaArgs(SYSTEM_TABLES.map(javaString), "            ");
  const contents = `// <auto-generated />
// Protocol constants projected from codegen/core/src/ir.ts (protocol
// ${ENGINE_V1}, docs/proposals/rule-parameters-as-data.md). Kept in sync
...
`;
  return { path: PROTOCOL_FILE, contents };
}

/**
 * Greedily wrap already-quoted Java argument literals onto MAX_LINE-bounded
 * lines at a fixed indent, so a long Set.of(...) stays readable in the
 * generated source.
 */
function wrapJavaArgs(args: string[], indent: string): string {
  const lines: string[] = [];
  let current = "";
  for (const arg of args) {
    const candidate = current === "" ? arg : `${current}, ${arg}`;
    if (current !== "" && indent.length + candidate.length > MAX_LINE) {
